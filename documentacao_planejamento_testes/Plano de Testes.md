...
#### Métodos do módulo de verificação de restrições da API da Sicred:
...
#### Métodos do módulo de gerenciamento de clientes da API da Sicred:    
...
![API Sicred](https://gitlab.com/pejnr/challenge_final_desafio_sicred/-/raw/docs/documentacao_planejamento_testes/images/mapa_mental/%5BAPI%5D%20SICRED2.png?ref_type=heads)     
...
- Instalação local da API Sicred.     

## 8.	CRITÉRIOS USADOS

**Que teste será feito:**
<p align="justify">Serão feitos testes de funcionalidade da API da Sicred, serão testadas as seguintes funções da API:</p>
...
**Como será feito:**    
1. Escolher um dos métodos da API Sicred podendo ser de Restrições ou Simulações (Por exemplo: O método GET de restrições ou de simulações);
...
**Falha da internet** – Pode ser contornado instalando o software de teste (Postman) e a API Sicred localmente no computador em que os testes serão realizados.    
...
<p align="justify">Os resultados dos testes serão divulgados através de um Board no Jira e quaisquer bugs e Issues serão divulgadas tanto no gitlab quanto no Jira.</p>

## 11.	CRONOGRAMA    

![Cronograma](https://gitlab.com/pejnr/challenge_final_desafio_sicred/-/raw/docs/documentacao_planejamento_testes/images/cronograma/cronograma_sicred.jpg?ref_type=heads)
...
2. Realizar uma operação `GET` no endpoint de restrições da API da Sicred informando o CPF escolhido (independente da porta a qual está sendo executada o endpoint será: `<host>/api/v1/restricoes/{cpf}`);
...
2. Realizar uma operação `POST` no endpoint de simulações da API da Sicred enviando o body como especificado acima (independente da porta a qual está sendo executada o endpoint será: `<host>/api/v1/simulacoes`);    
3. Obter e analisar a resposta obtida através da requisição. 
...
3. Realizar uma operação `POST` no endpoint de simulações da API da Sicred enviando o body como especificado acima (independente da porta a qual está sendo executada o endpoint será: `<host>/api/v1/simulacoes`);    
...
```    
2. A informação do CPF nessa requisição necessita ser o mesmo que uma das cadastradas anteriormente, no caso basta realizar uma requisição `POST` no endpoint de simulações da API da Sicred enviando o body como especificado acima e em seguida realizar outra sem alterar as informações enviadas (independente da porta a qual está sendo executada o endpoint será: `<host>/api/v1/simulacoes`);   
3. Obter e analisar a resposta obtida através da requisição.    
...
**Passo a passo para realização do teste:**     
1. Realizar uma operação `GET` no endpoint de simulações da API da Sicred (independente da porta a qual está sendo executada o endpoint será: `<host>/api/v1/simulacoes`);
...
**Passo a passo para realização do teste:**     
1. Garantir que todos os registros anteriores de simulação tenham sido excluídos realizando uma operação `DELETE` no endpoint de simulações da API da Sicred informando o ID de simulação a ser excluído (independente da porta a qual está sendo executada o endpoint será: `<host>/api/v1/simulacoes/{id}`);
1. Realizar uma operação `GET` no endpoint de simulações da API da Sicred informando (independente da porta a qual está sendo executada o endpoint será: `<host>/api/v1/simulacoes`);
2. Obter e analisar a resposta obtida através da requisição.    
...
2. Realizar uma operação `GET` no endpoint de simulações da API da Sicred informando o CPF escolhido (independente da porta a qual está sendo executada o endpoint será: `<host>/api/v1/simulacoes/{cpf}`);
3. Obter e analisar a resposta obtida através da requisição.    
...
3. Realizar uma operação `PUT` no endpoint de simulações da API da Sicred enviando o body como especificado acima e informando o CPF da simulação a ser alterada ao final do endpoint (independente da porta a qual está sendo executada o endpoint será: `<host>/api/v1/simulacoes/{cpf}`);    
4. Obter e analisar a resposta obtida através da requisição. 
...
```    
2. Realizar uma operação `PUT` no endpoint de simulações da API da Sicred enviando o body como especificado acima e informando o CPF da simulação a ser alterada ao final do endpoint (independente da porta a qual está sendo executada o endpoint será: `<host>/api/v1/simulacoes/{cpf}`);    
3. Obter e analisar a resposta obtida através da requisição. 
...
2. Realizar uma operação `DELETE` no endpoint de simulações da API da Sicred informando o ID da simulação cadastrada (independente da porta a qual está sendo executada o endpoint será: `<host>/api/v1/simulacoes/{id}`);    
3. Obter e analisar a resposta obtida através da requisição.    
...
2. Realizar uma operação `DELETE` no endpoint de simulações da API da Sicred informando um ID não existente nas simulações cadastradas (independente da porta a qual está sendo executada o endpoint será: `<host>/api/v1/simulacoes/{id}`);    
3. Obter e analisar a resposta obtida através da requisição.    
...
<p align="justify">Dos cenários base citados acima bem como os elencados no Jira, os testes que possuem maior relevância são incialmente os de cadastro do endpoint de simulações, pois sem esta funcionalidade nenhuma das outras funções segue a funcionar, exceto a de restrições que não possui nenhuma ligação direta com as simulações, visto isso a prioridade seria automatização dos testes de cadastro para tomar menos tempo, recursos e verificar com mais precisão a funcionalidade a fim de atingir uma área maior de testes.</p>

Aqui é possível acessar todos os testes gerados bem como toda a informação da Sprint: 
[**Acessar Board do Jira**](https://id.atlassian.com/invite/p/jira-software?id=lim9hmvkRmC35bowHLMkhA)